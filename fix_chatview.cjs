...
// And also activeThread.participant without dot something might be problematic. Let's fix activeThread.participant! where it's passed as object:
// onClick={() => onOpenUserProfile && onOpenUserProfile(activeThread.participant!)}
// We'll leave those for a manual patch or just replace with an empty object for groups, but they shouldn't be called for groups because there's no profile for a group.
